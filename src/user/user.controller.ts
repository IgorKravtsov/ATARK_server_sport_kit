...
import { IOrganizationResponse as OrganizationResponse } from './interfaces/IOrganizationResponse';
...
export class UserController {
  ...
  async getUserOrganizations(
    ...
    return organizations.map((organization) =>
      this.userService.buildOrganizationResponse(organization),
    );
  }
  ...
  @Get('characteristics')
  @UseGuards(AuthGuard)
  async getUserCharacteristics(@AuthUser() user): Promise<User> {
    return this.userService.getUserCharacteristics(user);
  }
  ...
  async updateCurrentUser(
    @AuthUser() user: User,
    @Body() updateUserDto: RegistrationDto,
  ): Promise<User> {
  ...
  async makeTrainer(@Param('learnerId') learnerId: number) {
    const updatedUser = await this.userService.makeTrainerFromLearner(
      learnerId,
    );
