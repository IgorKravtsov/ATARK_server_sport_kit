...
export class TrainingService {
  ...
  async getAllTrainings(): Promise<Training[]> {
    return await this.trainingRepository.find({relations: ['users', 'trainer']})
  }

  async getAllTrainingsInSomePeriod(currentUser: User, getPeriodDto: TrainingGetPeriodDto): Promise<Training[]> {
    const {periodInDays, thisWeek} = getPeriodDto
    const nowDate = new Date()

    if(periodInDays) {
      const period = Between(nowDate, addDays(nowDate, parseInt(periodInDays)))
      return await this.trainingRepository.find({where: {trainingDate: period}})
    }
    ...
    if(thisWeek) {
      ...
      console.log("TRAINER", currentUser);
      return await this.trainingRepository.find({where: {trainingDate: period, trainer: currentUser.trainer}})
    }

    throw new HttpException("Не указан ни один из параметров поиска", HttpStatus.BAD_REQUEST)
  }

  async createTraining(trainer: User, createTrainingDto: TrainingCreateDto): Promise<Training> {
    ...
    if(!gym) {
      throw new HttpException(`Зал с id: ${gymId} не найден`, HttpStatus.NOT_FOUND)
    }
    training.gym = gym
